...
@app.route("/patients/<int:pid>/details", methods=["GET"])
def patient_details(pid):
    """
    ดึงข้อมูลผู้ป่วยครบถ้วนสำหรับแสดงใน popup
    รวม: ชื่อ-นามสกุล, วันเกิด, เลขบัตรประชาชน, เพศ, ที่อยู่, เบอร์, เบอร์โทรฉุกเฉิน, กรุปเลือด, โรคประจำตัว
    """
    cur = get_cursor()
    if not cur:
        return jsonify({'status':'error','message':'DB error'}), 500
    ...
    try:
        # ดึงข้อมูลผู้ป่วย (ใช้ SELECT * เพื่อดึงทุก field)
        cur.execute("SELECT * FROM patient WHERE patient_id = %s", (pid,))
        patient = cur.fetchone()
        
        if not patient:
            cur.close()
            return jsonify({'status':'error','message':'ไม่พบข้อมูลผู้ป่วย'}), 404
        ...
        result = {
            ...
            'birth_date': patient.get('birth_date').strftime('%Y-%m-%d') if patient.get('birth_date') else '',
            'id_card': patient.get('id_card') or patient.get('national_id') or '',
            'gender': patient.get('gender', ''),
            'address': patient.get('address', ''),
            'phone': patient.get('phone', ''),
            'emergency_phone': patient.get('emergency_phone') or patient.get('emergency_contact') or '',
            'blood_group': patient.get('blood_group') or patient.get('blood_type') or '',
            'chronic_diseases': patient.get('chronic_diseases') or patient.get('underlying_diseases') or patient.get('medical_history') or ''
        }
        ...
        print(f"patient_details error: {e}")
        try:
            cur.close()
        except Exception:
            pass
        return jsonify({'status':'error','message':'เกิดข้อผิดพลาดในการดึงข้อมูลผู้ป่วย'}), 500
...
if __name__ == '__main__':
    ...
    print(">>> Server จะรันที่ http://127.0.0.1:5000")
    print(">>> ตรวจสอบว่าฐานข้อมูล MySQL ทำงานอยู่")
    
    try:
        app.run(debug=True, host="0.0.0.0", port=5000, threaded=True)
    except OSError as e:
        if "Address already in use" in str(e):
            print(f">>> ข้อผิดพลาด: Port 5000 ถูกใช้งานอยู่แล้ว")
            print(f">>> ลองใช้คำสั่ง: lsof -ti:5000 | xargs kill -9")
            print(f">>> หรือเปลี่ยน port ในไฟล์ api.py")
        else:
